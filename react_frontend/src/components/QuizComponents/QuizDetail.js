...
const QuizDetail = () => {
  ...
  useEffect(() => {
    const fetchQuizData = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/assignments/fetch-quiz-data/${quizId}/`, {
          headers: {
  ...
  if (!quizData || !quizData.questions || quizData.questions.length === 0) return <div>No quiz data available</div>;
