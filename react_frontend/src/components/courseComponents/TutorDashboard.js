...
const TutorDashboard = () => {
	...
	return (
		<div className="mx-auto mt-10 lg:w-[90%]">
			<h1 className="text-2xl font-bold mb-4 md:ml-[275px] text-center text-cyan-950">
				My Courses
			</h1>
			{courses.length === 0 ? (
				...
				<ul className="container md:w-[60%] md:ml-[275px] lg:ml-[260px] lg:grid lg:grid-cols-4 lg:w-[80%]">
					{courses.map((course) => (
						<li
							key={course.id}
							className="bg-white w-[90%] shadow-lg items-center justify-center mx-auto rounded lg:rounded-3xl  mb-4 dark:bg-gray-800 dark:text-gray-200 lg:w-[300px]"
						>
							<img
							...
							/>
							<h2 className="text-xl font-bold mb-2 pt-2 p-1">
								{course.course_name}
							</h2>
							<p className="text-sm text-gray-500 px-2">
								Course Creator: {course.teacher_name}
							</p>
							<p className="text-gray-700 line-clamp-3 dark:text-gray-300 p-2">
								{course.course_description}
							</p>
							<div className="flex gap-2 flex-row pt-4 pb-2">
								<p className="text-sm text-gray-500 py-3 p-2">
									Enrolled: {course.teacher}
								</p>
								<a
									href="/update-course"
